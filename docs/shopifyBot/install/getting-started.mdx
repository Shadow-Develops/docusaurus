...
This document will help you get your server setup and ready to get Shopify Bot running on it!

:::warning[Reminders]
Keep in mind all install guides are for an Ubuntu 24.04 LTS Linux VPS.
:::

Make sure that you have read through our [prerequisites](/shopifyBot/install/prerequisites) to prevent any issues later on.\
...
[Node.Js](https://nodejs.org) is a very popular framework for Javascript used to manage many different types of applications. Shopify Bot is a Discord.JS and ExpressJS application that requires it to be run, and since Node.Js has many versions we will be using Node Package Manager (NVM) to ensure the correct version is installed and running on our system.
...
[Nginx](https://nginx.org/) is the program that we will use to route and proxy the Shopify Bot's ExpressJS web server so it properly appears on your domain.
...
[MongoDB](https://www.mongodb.com/) is the database program which Shopify Bot uses to store all its data. MongoDB is a JSON based database which makes it easy to read and understand.

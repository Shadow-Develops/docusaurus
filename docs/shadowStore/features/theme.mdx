...
Shadow Store supports two theme modes:

### Single Theme

One active theme is applied sitewide. No toggle is shown to visitors. This is the default mode.

### Dual Mode

Two themes are configured, a primary and a secondary, and visitors can switch between them using a toggle button. This is useful for offering a light and dark variant of your store.

**Toggle Settings (Dual Mode only):**

| Setting             | Options                           |
| ------------------- | --------------------------------- |
| **Toggle Position** | Navbar, Footer, Both, or Hidden   |
| **Button Style**    | Icon only, Name only, Icon + Name |

Each theme in Dual Mode is configured independently (Preset or Custom) with its own colors, custom CSS, and settings.

## Theme Types

Shadow Store offers two approaches to theming for each configured theme:

### Preset Themes

Choose from 32 professionally designed daisyUI themes:

**Light Themes:**

- Light, Cupcake, Bumblebee, Emerald, Corporate, Wireframe, Lo-Fi, Pastel, Fantasy, CMYK, Autumn, Lemonade, Winter
...
:::info
New seasonal effects are added periodically. Join the [Discord](https://discord.gg/fVrRa8z) to suggest new effects!
:::

## Per-Theme Custom CSS

Each configured theme (primary and secondary in Dual Mode) has its own **Custom CSS** field. CSS entered here only applies when that specific theme is active. This lets you make targeted adjustments per theme without affecting the other.

**To Add Per-Theme Custom CSS:**

1. Navigate to **Admin Panel > Theme**
2. Expand the theme you want to customize
3. Scroll to the **Custom CSS** field for that theme
4. Enter your CSS
5. Click "Save Theme Settings"

## Global CSS

Inject CSS that applies to all pages regardless of which theme is active:

**To Add Global CSS:**

1. Navigate to **Admin Panel > Theme**
2. Scroll to the **Global CSS** section
3. Enter your CSS in the textarea
4. Click "Save Theme Settings"

**Available CSS Variables (usable in both per-theme and global CSS):**
...
| ------------------- | -------------------------------------------------- |
| `owner`             | Full access                                        |
| `MANAGE_APPEARANCE` | Theme customization, custom CSS, branding settings |

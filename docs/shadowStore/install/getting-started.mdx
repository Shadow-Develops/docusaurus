...
[Node.Js](https://nodejs.org) is a very popular framework for Javascript used to manage many different types of applications. Shadow Store is one type of application that requires it to be run, and since Node.Js has many versions we will be using Node Package Manager (NVM) to ensure the correct version is installed and running on our system.
...
[Nginx](https://nginx.org/) is the program that we will use to route and proxy all Shadow Store so it properly appears to the world on your domain.
...
[MongoDB](https://www.mongodb.com/) is the database program which Shadow Store uses to store all its data. MongoDB is a JSON based database which makes it easy to read and understand.

...
This document will help you get your server setup and ready to get CDN Manager running on it!
...
Make sure that you have read through our [prerequisites](/cdnManager/install/prerequisites) to prevent any issues later on.\
...
[Node.Js](https://nodejs.org) is a very popular framework for Javascript used to manage many different types of applications. CDN Manager is one type of application that requires it to be run, and since Node.Js has many versions we will be using Node Version Manager (NVM) to ensure the correct version is installed and running on our system.
...
If you want to see if it has installed and is running the correct node version (24.11), you can run: `node --version` in your console.
...
[Nginx](https://nginx.org/) is the program that we will use to route and proxy CDN Manager so it properly appears to the world on your domain.
...
[MongoDB](https://www.mongodb.com/) is the database program which CDN Manager uses to store all its data. MongoDB is a JSON based database which makes it easy to read and understand.
